...
if [ ! -d "$SIM_DIRECTORY" ]; then
    ...
    docker run --rm -it --net=host --ipc=host --pid=host --privileged -v /dev/shm:/dev/shm -e DISPLAY=$DISPLAY -v /dev/input:/dev/input:rw -v /tmp/.X11-unix:/tmp/.X11-unix:ro -v $SIM_DIRECTORY:/home/root:rw -w /home/root --name=ros2humble ghcr.io/ufl-autonomy-park/apark-ros2:mac /bin/bash -l
