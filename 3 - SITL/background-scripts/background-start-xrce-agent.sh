...
if [ ! "$(docker ps -a | grep "$CONTAINER_NAME")" ]; then
	...
	docker exec -d $CONTAINER_NAME bash -c "source /home/root/ros-sources.sh; MicroXRCEAgent udp4 -p 8888 2>&1 | tee >(head -n 200 > /tmp/xrce-output.log) > /dev/null "
	sleep 10
	# docker exec -i $CONTAINER_NAME bash -c "cat /tmp/xrce-output.log"
fi
