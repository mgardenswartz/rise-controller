...
def phase_1_tune_baselines():
    print("="*50 + "\nPHASE 1: NATIVE OPTUNA TUNING (IN-PROCESS)\n" + "="*50)
    
    # Supress massive Optuna logging noise
    ...
    for sys_id in SYSTEMS:
        ...
        def objective(trial):
            ...
            try:
                # Diffrax solver is entirely silent here to avoid progress bar spam
                config.simulation.debug_print = False
                sim_data = run_simulation(config)
                
                # Calculate RMS tracking error natively
                e = sim_data[config.data_labels.tracking_error]
                cost = float(jnp.sqrt(jnp.mean(jnp.sum(e**2, axis=-1))))
                return cost
            except RuntimeError:
                # If a combination of gains causes a finite-time escape, prune it cleanly
                raise optuna.TrialPruned()

        # Seed 42 guarantees repeatable baselines across runs
        study = optuna.create_study(direction="minimize", sampler=optuna.samplers.TPESampler(seed=42))
        
        # Execute 50 trials. JIT caching makes this incredibly fast.
...
def phase_2_massive_sweep(gains_dict: dict):
    ...
    controllers = [("baseline", 2), ("nn_in_integral", 4)]
            ...
            for size_name, target_p in TARGET_PARAMS.items():
                ...
                for i in range(MC_TRIALS):
                    ...
                    try:
                        ...
                        print(f"  -> Trial {i+1} FAILED (Finite Escape / Stiff Dynamics)")
                
                jax.clear_caches()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Master Sweep for 2D Systems")
